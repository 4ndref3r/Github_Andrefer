<?php  
    ...
    if (isset($this->d['editar']) && $this->d['editar'] !== NULL) {
        $editar = $this->d['editar'];
    }
?>
<?php  require_once "./app/views/inc/head.php"; ?>
<?php  require_once "./app/views/inc/admin_users.php"; ?>
                    ...
                    <div class="inline-space mt-5 flex">
                        ...
                        <template x-teleport="#x-teleport-target">
                            ...
                            <div class="relative flex w-full max-w-lg origin-top flex-col overflow-hidden rounded-lg bg-white transition-all duration-300 dark:bg-navy-700" x-show="showModal" x-transition:enter="easy-out" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="easy-in" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95">
                                ...
                                <form action="<?php echo constant('URL'); ?>admin/sendConfirmationEmail" method="post">
                                <div class="mt-4 space-y-4">
                                    <span>Correo:</span>
                                    <input name="usuario" class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" placeholder="Ingresa el correo" type="text" autocomplete="off">
                                    </label>
                                    <label class="block">
                                    <span>Nombre:</span>
                                    <input name="contra" class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" placeholder="Ingresa el nombre" type="text">
                                    </label>
                                    <label class="block">
                                    <span>Rol:</span>
                                    <select name="rol" class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
                ...
                <div class="mt-5">
                    ...
                    <table class="is-zebra w-full text-left">
                        ...
                        <tr>
                            ...
                            <td class="whitespace-nowrap px-4 py-3 sm:px-5"><input type="hidden" name="id" value="<?php echo $usuario->getId();?>">
                                <?php echo $usuario->getUsuario(); ?>
                            </td>
                            <td class="whitespace-nowrap px-4 py-3 sm:px-5"><?php echo $usuario->getRol(); ?></td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium flex items-center space-x-3">
                                <!-- Botón de Editar -->
                                <a href="<?php echo constant('URL').'admin/admValues/'.$usuario->getId(); ?>" @click="$dispatch('open-modal', { id: <?php echo $usuario->getId(); ?>" class="text-blue-600 hover:text-blue-900" title="Modificar">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6">
                                        <path d="M21.731 2.269a2.625 2.625 0 0 0-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 0 0 0-3.712ZM19.513 8.199l-3.712-3.712-8.4 8.4a5.25 5.25 0 0 0-1.32 2.214l-.8 2.685a.75.75 0 0 0 .933.933l2.685-.8a5.25 5.25 0 0 0 2.214-1.32l8.4-8.4Z" />
                                    </svg>
                                </a>
                                <?php if($usuario->getEstado() == 1): ?>
                                <a href="<?php echo constant('URL').'admin/admDelete/'.$usuario->getId(); ?>" class="text-red-600 hover:text-red-900" title="Deshabilitar">
                                ...
                                <?php else: ?>
                                <a href="<?php echo constant('URL').'admin/admHabilite/'.$usuario->getId(); ?>" class="text-red-600 hover:text-red-900" title="Habilitar">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
